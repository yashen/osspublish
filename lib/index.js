...
var remoteVersionFile = ".remoteVersion";
...
upload.option("-k,--key <accessKeyId>", "accessKeyId")
    ...
    .option("-f --force","force upload when remote version is same");
...
upload.action(function () {
    ...
    var config = loadConfig();
    if(!upload.force && fs.existsSync(remoteVersionFile)){
        var remoteVersion = new String(fs.readFileSync(remoteVersionFile));
        if(remoteVersion == config.version){
            console.log("Remote version is same " + config.version + ",not Upload")
            return;
        }
    }
    ...
    co(function* () {
        ...
        if (!upload.skipUpload) {
            ...
            config.versions[config.version] = result.url;
            ...
            config.qrcode = qrCodeResult.url;
        ...
        fs.writeFileSync(remoteVersionFile,config.version);
